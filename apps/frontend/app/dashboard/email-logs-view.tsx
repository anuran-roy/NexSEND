...
export function EmailLogsView({
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      case "SENT":
        return "border-[var(--success)] text-[var(--success)] bg-[var(--success)]/10";
      case "FAILED":
      case "BOUNCED":
      case "COMPLAINED":
        return "border-[var(--danger)] text-[var(--danger)] bg-[var(--danger)]/10";
      case "PROCESSING":
        return "border-[var(--primary)] text-[var(--primary)] bg-[var(--primary)]/10";
      case "QUEUED":
        return "border-[var(--warning)] text-[var(--warning)] bg-[var(--warning)]/10";
      default:
        return "border-[var(--muted)] text-[var(--muted)] bg-[var(--muted)]/10";
    }
  };
          ...
          <div className="flex items-start justify-between flex-wrap gap-4 mb-6">
            <div>
              ...
              <p className="caption mt-1">
                Trace every email sent through the system.
              </p>
            </div>
        ...
        <div className="p-0 bg-[var(--card)]">
          {logs.length === 0 && !loading ? (
            <div className="p-10 text-center flex flex-col items-center">
              ...
              <p className="caption">
                No email jobs found matching the current filters.
              </p>
            </div>
          ) : (
                ...
                <TableBody>
                  {logs.map((log) => (
                    <React.Fragment key={log.id}>
                      <TableRow
                        className="group cursor-pointer hover:bg-[var(--background-elevated)]"
                        onClick={() =>
                      ...
                      >
                        ...
                        <TableCell className="font-mono text-xs text-[var(--primary)]">
                          {log.id.slice(0, 8)}...{log.id.slice(-4)}
                        </TableCell>
                        <TableCell
                          className="font-medium max-w-[200px] truncate"
                          title={formatEmails(log.toEmails)}
                        >
